...
def _get_clustering_algorithms(n_clusters: int) -> Dict[str, Any]:
...
def cluster_users(
    ...
    results = {}
    scores = {}
    algorithms = _get_clustering_algorithms(n_clusters)
...
def compute_cluster_scores(
    X: np.ndarray, min_clusters: int = 2, max_clusters: int = 10
) -> pd.DataFrame:
    ...
    for k in tqdm(
        ...
        _, scores = cluster_users(X, n_clusters=k, return_scores=True)
        results.extend(
            [
